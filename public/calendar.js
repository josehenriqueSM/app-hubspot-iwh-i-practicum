...
async function createCalendar() {
  //TODO: Wrap everything in a try/catch for error handling.
    ...
    if (resp.status === 200 ) {
        ...
        document.getElementById('month-heading').innerHTML = `${new Date(month).toLocaleString('en-us', {month:'long', year:
    'numeric'})}`;
    }
    else {
        throw new Error(`Something wenf wrong: ${resp.statusText}`)
    }
